...
export function onOptimizePromptChunk(cb: (tabId: string, chunk: string) => void): () => void {
  const off = hostEvents("prompt-optimize:chunk", (tabId?: unknown, chunk?: unknown) => {
...
export function onOptimizePromptDone(cb: (tabId: string) => void): () => void {
  const off = hostEvents("prompt-optimize:done", (tabId?: unknown) => {
...
// drive the same listener path the real host event stream uses.
    ...
    async RemovePermissionRule(list: string, rule: string) {
      ...
      settings.permissions[k] = settings.permissions[k].filter((r) => r !== rule);
        ...
        async SetDesktopLayoutStyle(style: string) {
          settings.desktopLayoutStyle = style === "workbench" || style === "creation" || style === "split" ? style : "workbench";
        },
